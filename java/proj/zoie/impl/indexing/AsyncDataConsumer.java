...
public class AsyncDataConsumer<D, V extends ZoieVersion> implements DataConsumer<D, V> 
{
  ...
  {
    //System.out.println("Hao: AsyncDataConsumer:SyncWthVersion: _currentVersion: " + _currentVersion + ", version: " + version);
    if(_consumerThread == null) throw new ZoieException("not running");
    
    synchronized(this)
    {
      //while(_currentVersion < version)
      //System.out.println(" AsyncDataConsumer:SyncWthVersion: _currentVersion: " + _currentVersion.encodeToString() + ", version: " + version.encodeToString());
//      if(_currentVersion==null)
//      {
//       // DefaultZoieVersion dzv = (DefaultZoieVersion) version;
//        //System.out.println("Hao: AsyncDataConsumer:SyncWthVersion: _currentVersion is null while version is  " + dzv.getVersionId());
//      }
//      else
//      {
//      
//        System.out.println("Hao: AsyncDataConsumer:SyncWthVersion: _currentVersion<version: " + _currentVersion.compareTo(version) );
//      }
...
//        }       
        ...
        try
        {
          long timetowait = Math.min(100, timeInMillis);
